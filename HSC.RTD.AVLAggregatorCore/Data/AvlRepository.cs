...
namespace HSC.RTD.AVLAggregatorCore.Data
{
    public class AvlRepository : IAvlRepository
    {
        ...
        // add new ServiceAccountService from IEnumerable
        public void newServiceAccountServices(IEnumerable<POCO.ServiceAccountService> SAS)
        {
            using (var conn = new SqlConnection(connString))
            {
                foreach (POCO.ServiceAccountService s in SAS)
                {

                    //conn.Execute("INSERT INTO Devices (Id, Address, VehicleID, AddedDateTime, AddedBy, ModifiedDateTime, ModifiedBy) VALUES (@Id, @Address, @VehicleID, CURRENT_TIMESTAMP, @AddedBy, Null, Null)", 
                    conn.Execute("INSERT INTO ServiceAccounts_Services (ServiceAccountId, ServiceId, AddedDateTime, AddedBy) VALUES (@ServiceAccountId, @ServiceId, CURRENT_TIMESTAMP, @AddedBy)",
                        new { s.ServiceAccountId, s.ServiceId , s.AddedBy});

                }
            }
        }

        // delete ServiceAccountService from IEnumerable
        public void deleteServiceAccountServices(IEnumerable<POCO.ServiceAccountService> SAS)
        {
            using (var conn = new SqlConnection(connString))
            {
                foreach (POCO.ServiceAccountService s in SAS)
                {
                    conn.Execute("DELETE FROM ServiceAccounts_Services WHERE ServiceAccountId = @ServiceAccountId AND ServiceId = @ServiceId", new { s.ServiceAccountId, s.ServiceId });
